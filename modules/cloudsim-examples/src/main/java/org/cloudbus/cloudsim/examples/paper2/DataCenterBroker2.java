...
import static org.cloudbus.cloudsim.examples.paper2.Constant2.*;
...
public class DataCenterBroker2 extends SimEntity {
     ...
     * @post $none
     */
    public DataCenterBroker2(String name) throws Exception {
        super(name);
    ...
    protected void submitCloudlets() {
        ...
        List<CustomCloudlet> HighPriorityCloudlets = new ArrayList<CustomCloudlet>();
        List<CustomCloudlet> MediumPriorityCloudlets = new ArrayList<CustomCloudlet>();
        List<CustomCloudlet> LowPriorityCloudlets = new ArrayList<CustomCloudlet>();

        double wait;

        for (CustomCloudlet cloudlet : getCloudletList()) {
            wait = cloudlet.getDeadline() - CloudSim.clock();
			if(wait <= (double) cloudlet.getCloudletLength() /1000){
				HighPriorityCloudlets.add(cloudlet);
			}
			else if((double) cloudlet.getCloudletLength() /1000 + 300 <= wait && wait <= (double) cloudlet.getCloudletLength() /1000 + 800){
				if(cloudlet.getPriority() == 1) HighPriorityCloudlets.add(cloudlet);
				else if(cloudlet.getPriority() == 2) MediumPriorityCloudlets.add(cloudlet);
				else if(cloudlet.getPriority() == 3) LowPriorityCloudlets.add(cloudlet);
			}
			else if(wait > (double) cloudlet.getCloudletLength() /1000 + 800){
				if(cloudlet.getPriority() == 1 && HighPriorityCloudlets.size() < Qh) HighPriorityCloudlets.add(cloudlet);
				else if(cloudlet.getPriority() == 1 && HighPriorityCloudlets.size() >= Qh) MediumPriorityCloudlets.add(cloudlet);
				else if(cloudlet.getPriority() == 2 && MediumPriorityCloudlets.size() < Qm) MediumPriorityCloudlets.add(cloudlet);
				else if(cloudlet.getPriority() == 2 && MediumPriorityCloudlets.size() >= Qm) LowPriorityCloudlets.add(cloudlet);
				else if(cloudlet.getPriority() == 3) LowPriorityCloudlets.add(cloudlet);
			}
        }
        ...
        int vmIndex = 0;
        List<CustomCloudlet> successfullySubmitted = new ArrayList<CustomCloudlet>();
        for (CustomCloudlet cloudlet : getCloudletList()) {
            Vm vm;
